...
Survey-derived proportion columns sometimes contain garbage values greater than
1. `replace_over_one_with_na()` replaces them with `NA` in one call.
...
Analysis scripts often need weighted proportions, means, or medians of survey
variables, optionally broken out by a grouping variable like size stratum or
region. `calc_summarize()` handles one variable; `summarize_by_groups()` runs it
over many `variable = metric` pairs and stacks the results --- replacing the
typical `purrr::map2() |> list_rbind()` pattern.
...
Drop `group_var` for a national (ungrouped) summary --- the `group` column will
contain `"National"`.
...
| `replace_over_one_with_na()` | Numeric columns | Numeric with NAs for > 1 | `vars` |
| `calc_summarize()` | Survey df + var + weight | Weighted summary tibble | `metric` |
| `summarize_by_groups()` | Survey df + named `var = metric` list | Stacked summaries | `group_var` |
